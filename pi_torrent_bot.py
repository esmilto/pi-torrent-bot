...
class TorrentBot(telepot.aio.Bot):
    ...
    async def on_chat_message(self, msg):
        ...
        if 'entities' in msg:
            ...
            if magnet_link.check_magnet_link(message) is not None:
                ...
                await self.sendMessage(chat_id, "Link has been added to queue")
                return
        ...
        if content_type == 'document':
            ...
            await self.sendMessage(chat_id, f"Added to queue: {file_name}")
            return
    ...
    def get_torrents_for_select(action='stop'):
        ...
        keyboard_ids = list([InlineKeyboardButton(text=torrents_out[id],
                                                  callback_data=f"{action}_{id}") for id in torrents_out])
    ...
    async def send_torrents_list(self, chat_id=None, message=''):
        ...
        await self.sendMessage(chat_id, f"{message}*ID\tDone\tStatus\tName*\n{torrents_list}", parse_mode='markdown')
...
pi_torrent_bot = TorrentBot(token=telegram_bot['token'])

if __name__ == '__main__':
    loop = asyncio.get_event_loop()
    loop.create_task(pi_torrent_bot.message_loop())
    loop.run_forever()
